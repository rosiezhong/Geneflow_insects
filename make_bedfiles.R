...
for (i in 1: nrow(merged_pos)) {
  if (is.na(merged_pos$counter2[i])) {
    ...
    while (length(queue) > 0) {
      ...
      for (j in 1: nrow(merged_pos)) {
        if (i != j && is.na(merged_pos$counter2[j])) {
          if (length(intersect(merged_pos$counter[[current]], merged_pos$counter[[j]])) > 0) {
...
final_pos<-data.frame(pos_start=numeric(0),pos_end=numeric(0))
for (i in 1:nrow(vcf_fix)) {
  for (j in 1:nrow(grouped_pos)) {
    if (vcf_fix$POS[i]>=grouped_pos$pos_start[j] & vcf_fix$POS[i]<=grouped_pos$pos_end[j]) {
      final_pos<-rbind(final_pos,grouped_pos[j,])
    }
  }
}
